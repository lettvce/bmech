"""
Pure geometry core for helical_gear.py - build_helical_gear_mesh is plain
Python + geo_helpers math, zero bpy/bmesh/mathutils dependency. Extracted
verbatim from helical_gear.py for the web configurator (Pyodide) - see
that file's own module docstring for the twist-per-slice technique this
...
# ── Involute math (verbatim from helical_gear.py) ──────────────────────
...
def build_gear_mesh(profile_points, width_mm, bore_r, ha_rad, pitch_r,
                     hand_sign, n_slices):
    """Loft the (already twisted-per-slice) tooth profile through
    `n_slices` Z rings, then cap the ends - see helical_gear.py's own
    ...
    for k in range(n_slices):
        ...
        pts   = [(x * c - y * s, x * s + y * c, z) for x, y in profile_points]
        rings.append(gh.Loop(pts, True, "hg_ring_%d" % k))
...
def derive_helical_gear(module, tooth_count, pressure_angle_deg, helix_angle_deg,
                         width_mm, bore_enable, bore_diameter, bore_compensation):
    """Pure derivation, matching helical_gear.py's own _derived()/draw()
    logic - clamps pressure_angle_deg to this tooth count's own ceiling,
    reports pitch/dedendum/addendum radii, total twist, normal module,
    and the resolved bore radius. Never raises."""
    ...
    ded_r         = pitch_r - DEDENDUM_COEFF * module
    add_r         = pitch_r + ADDENDUM_COEFF * module
    total_twist   = width_mm * tan(ha_rad) / pitch_r
    ...
    return dict(
        ...
        pitch_r=pitch_r, ded_r=ded_r, add_r=add_r, total_twist=total_twist,
        normal_module=normal_module, bore_r=bore_r,
    )


def build_helical_gear_mesh(module=2.0, tooth_count=20, pressure_angle_deg=20.0,
                             helix_angle_deg=20.0, hand='RIGHT', width_mm=10.0,
                             bore_enable=True, bore_diameter=5.0, bore_compensation=0.2,
                             n_slices=16):
    """Returns (mesh, derived_dict) - a closed, single-solid helical gear,
    with an optional bore hole. Raises ValueError for invalid geometry
    (module too large for this tooth count)."""
    derived = derive_helical_gear(module, tooth_count, pressure_angle_deg, helix_angle_deg,
                                   width_mm, bore_enable, bore_diameter, bore_compensation)
    if derived["ded_r"] <= 0:
    ...
    if not report.is_valid:
        raise ValueError("Helical gear failed to build a closed shell: %s" % "; ".join(report.errors))
