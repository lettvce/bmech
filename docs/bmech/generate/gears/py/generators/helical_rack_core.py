"""
Pure geometry core for helical_rack.py - build_helical_rack_mesh is plain
Python + geo_helpers math, zero bpy/bmesh/mathutils dependency. Extracted
verbatim from helical_rack.py for the web configurator (Pyodide) - see
that file's own module docstring for the exact-affine-shear technique
(why only two Z layers are needed, unlike a helical gear's per-slice
rotation) this reproduces unchanged.
...
def build_helical_rack_mesh(profile_points, shear_x, width_mm):
    """Closed sheared-rack solid: the whole top ring is the bottom ring
    translated by (shear_x, 0, width_mm) - see helical_rack.py's own
    docstring for why this exact-affine shear only needs two Z layers.
    Raises ValueError if the resulting mesh fails validation."""
    profile = gh.Loop([(x, y, 0.0) for x, y in profile_points], True, "rack_profile")
    vec = (shear_x, 0.0, width_mm)
    top = gh.translate(profile, vec)

    mesh = (gh.extrude(profile, vec)
    ...
    if not report.is_valid:
        raise ValueError("Helical rack mesh failed validation: %s" % "; ".join(report.errors))
...
def derive_helical_rack(module, pressure_angle_deg, tooth_count_rack, helix_angle_deg, width_mm):
    """Pure derivation - a rack has no self-intersection pressure-angle
    ceiling. Never raises."""
    ha_rad = radians(helix_angle_deg)
    shear  = width_mm * tan(ha_rad)
    tooth_pitch = pi * module
    length_mm = tooth_count_rack * tooth_pitch
    return dict(ha_rad=ha_rad, shear=shear, tooth_pitch=tooth_pitch, length_mm=length_mm)


def build_helical_rack_mesh_full(module=1.0, pressure_angle_deg=20.0, tooth_count_rack=10,
                                  helix_angle_deg=20.0, hand='RIGHT', width_mm=10.0):
    """Returns (mesh, derived_dict) - a closed, single-solid helical
    (sheared-tooth) rack. Raises ValueError for invalid geometry."""
    derived = derive_helical_rack(module, pressure_angle_deg, tooth_count_rack, helix_angle_deg, width_mm)
    hand_sign = 1.0 if hand == 'RIGHT' else -1.0

    profile = build_rack_profile(module, pressure_angle_deg, tooth_count_rack)
    mesh = build_helical_rack_mesh(profile, hand_sign * derived["shear"], width_mm)
    return mesh, derived
