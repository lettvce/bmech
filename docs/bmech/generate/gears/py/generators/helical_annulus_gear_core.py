"""
Pure geometry core for helical_annulus_gear.py -
build_helical_annulus_gear_mesh is plain Python + geo_helpers math, zero
bpy/bmesh/mathutils dependency. Extracted verbatim from
helical_annulus_gear.py for the web configurator (Pyodide) - see that
file's own module docstring for the twisted-inner/plain-outer technique
...
def _build_annulus_cutter_profile(module, tooth_count, pa_deg):
    ...
    helical_annulus_gear.py's own docstring for the add/ded-swap
    rationale."""
...
def build_helical_annulus_mesh(inner_profile, width_mm, outer_r,
                                hand_sign, ha_rad, pitch_r, n_slices):
    """Build the complete helical annulus ring solid via geo_helpers -
    twisted inner toothed wall, plain outer cylindrical wall, and two
    annular end caps. See helical_annulus_gear.py's own docstring for the
    full rationale. Raises ValueError if the resulting mesh fails
    validation."""
    inner_slices = [
    ...
    if not report.is_valid:
        raise ValueError("Helical annulus mesh failed validation: %s" % "; ".join(report.errors))
    return mesh


# ── Derived values + top-level build ────────────────────────────────────

def derive_helical_annulus_gear(module, tooth_count, pressure_angle_deg, helix_angle_deg,
                                 width_mm, ring_wall_mm):
    """Pure derivation, matching helical_annulus_gear.py's own
    ...
    return dict(
        ...
        total_twist=total_twist, normal_module=normal_module,
    )


def build_helical_annulus_gear_mesh(module=2.0, tooth_count=40, pressure_angle_deg=20.0,
                                     helix_angle_deg=20.0, hand='RIGHT', width_mm=10.0,
                                     ring_wall_mm=5.0, n_slices=16):
    ...
    derived = derive_helical_annulus_gear(module, tooth_count, pressure_angle_deg,
    ...
    mesh = build_helical_annulus_mesh(inner_profile, width_mm, derived["outer_r"],
